// Table JS

var goalsName = [
    [
        '1',
        'Have to do Sidebar section',
        '<progress value="60" max="100"> 60% </progress>    <span>60%</span>',
        '<a href="../goals/edit_goals.html"><i class="fa fa-edit"></i></a>  |   <i class="fa fa-trash"></i>'
    ],
    [
        '2',
        'Manage Tasks Page',
        '<progress value="30" max="100"> 30% </progress>    <span>30%</span>',
        '<a href="../goals/edit_goals.html"><i class="fa fa-edit"></i></a>  |   <i class="fa fa-trash"></i>'
    ],
    [
        '3',
        'Work on design',
        '<progress value="70" max="100"> 70% </progress>    <span>70%</span>',
        '<a href="../goals/edit_goals.html"><i class="fa fa-edit"></i></a>  |   <i class="fa fa-trash"></i>'
    ],
    [
        '4',
        'Manage website traffic',
        '<progress value="50" max="100"> 50% </progress>    <span>50%</span>',
        '<a href="../goals/edit_goals.html"><i class="fa fa-edit"></i></a>  |   <i class="fa fa-trash"></i>'
    ],
    [
        '5',
        'Make website\'s design attractive',
        '<progress value="20" max="100"> 20% </progress>    <span>20%</span>',
        '<a href="../goals/edit_goals.html"><i class="fa fa-edit"></i></a>  |   <i class="fa fa-trash"></i>'
    ],
    [
        '6',
        'Work on registration',
        '<progress value="40" max="100"> 40% </progress>    <span>40%</span>',
        '<a href="../goals/edit_goals.html"><i class="fa fa-edit"></i></a>  |   <i class="fa fa-trash"></i>'
    ],
    [
        '7',
        'Login flow should be with OTP and password',
        '<progress value="80" max="100"> 80% </progress>    <span>80%</span>',
        '<a href="../goals/edit_goals.html"><i class="fa fa-edit"></i></a>  |   <i class="fa fa-trash"></i>'
    ],
    [
        '8',
        'Work on file upload',
        '<progress value="50" max="100"> 50% </progress>    <span>50%</span>',
        '<a href="../goals/edit_goals.html"><i class="fa fa-edit"></i></a>  |   <i class="fa fa-trash"></i>'
    ],
    [
        '9',
        'Work on calendar page',
        '<progress value="30" max="100"> 30% </progress>    <span>30%</span>',
        '<a href="../goals/edit_goals.html"><i class="fa fa-edit"></i></a>  |   <i class="fa fa-trash"></i>'
    ],
    [
        '10',
        'Manage lists Page',
        '<progress value="90" max="100"> 90% </progress>    <span>90%</span>',
        '<a href="../goals/edit_goals.html"><i class="fa fa-edit"></i></a>  |   <i class="fa fa-trash"></i>'
    ],
    [
        '11',
        'Work on index Page',
        '<progress value="60" max="100"> 60% </progress>    <span>60%</span>',
        '<a href="../goals/edit_goals.html"><i class="fa fa-edit"></i></a>  |   <i class="fa fa-trash"></i>'
    ]
];